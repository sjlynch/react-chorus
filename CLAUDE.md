...
Recurring CI failures: lint errors (`no-explicit-any`, `prefer-const`, `no-this-alias`), README bundle-size drift, and `examples/*` build failures. Lint runs first with `--max-warnings=0` and skips everything else on failure, so one error masks the rest. README bundle-size drift is the single most common red CI run, and merges from `main` cause it as often as direct `src/` edits — even a small (~1 kB) shift fails CI because `scripts/verify-bundle-size.mjs` does **exact string-match** of measured values against README, not a budget check. There are TWO spots to refresh, both under the `## Bundle size and tree-shaking` heading: (a) the `Current numbers:` table (`<Chorus>`, `react-chorus/headless`, `useChorusStream`, `Markdown`, `ChatWindow`, `ConversationList`, the three subpaths, and the lazy `highlight.js` row), and (b) the playground paragraph starting `The current playground initial JS graph is …`. Library numbers come from `npm run build && npm run verify:bundle-size`; the playground number requires the separate `npm run build:playground` (runs `verify:playground-size` automatically). Pull both sets from stdout or `.cache/react-chorus/{library,playground}-bundle-size-report.json`. After any merge that touches `src/` or runtime deps, re-run both verifications before pushing — don't trust that the prior README values still match. Example builds run each example's own `next build`/`vite build` against its pinned typings, so type refactors in `src/` (and the library's React 19 typings) can surface in examples even though the root typecheck passes; bump `@types/react`/`@types/react-dom` in an example if it pins v18 and hits a `ReactPortal`/`ReactNode` mismatch. For unavoidable `any` (e.g. `Connector<State = any>` bivariance), use a focused `// eslint-disable-next-line` with a one-line reason.
