...
| `composerFooter` | `ReactNode \| ((ctx: { sending: boolean }) => ReactNode)` | — | Host control(s) rendered directly beneath the composer input row, inside a `.chorus-composer-footer` element in the composer's vertical stack. A content-agnostic slot for a status/usage/throughput meter, a hint line, or any footer chrome. Pass a render-prop to react to the live `sending` state. Also accepted by the standalone `<ChatInput>`. |
...
**Flow typography is self-contained.** The injected `chorus-md` sheet states its own spacing for paragraphs, headings, lists, list items, blockquotes, and horizontal rules rather than inheriting it from the browser's UA stylesheet, because host apps routinely ship a reset (`* { margin: 0; padding: 0 }`, normalize.css, Tailwind preflight) that strips it. Without these rules `list-style-position: outside` markers paint to the left of the list's content box — i.e. outside the bubble's padding, on or past its border — every nesting level collapses onto the same indent, and blocks lose their vertical rhythm. Lists get `padding-inline-start: 2em` (enough for two-digit ordinals) plus explicit `disc`/`circle`/`square`/`decimal` markers, so a `list-style: none` reset cannot erase them either. Three CSS variables theme the result: `--chorus-md-quote-border` and `--chorus-md-quote-text` (blockquote rule and text) and `--chorus-md-rule` (`<hr>` color). Override them alongside the other `--chorus-*` variables (see [Theming](#theming)). Spacing is `em`-relative, so it scales with whatever font size the surrounding surface sets. The headless build injects no stylesheet — style these elements yourself there.

**Link hardening (reverse tabnabbing).** When the built-in DOMPurify sanitizer resolves (no custom `sanitizer` override), Chorus registers a one-time DOMPurify `afterSanitizeAttributes` hook that forces `rel="noopener noreferrer"` on any rendered anchor that opens a new browsing context — `target="_blank"` or a named target — so the opened page cannot reach `window.opener` (CWE-1022). Existing `rel` tokens are preserved and a conflicting `opener` token is dropped; anchors targeting the current context (`_self`/`_parent`/`_top`) or with no `target` are left untouched. DOMPurify's default config already strips `target`, so in the default pipeline no anchor opens a new context and the hook is a no-op safety net; it engages automatically if a resolved DOMPurify is configured to keep `target` (e.g. a consumer `ADD_ATTR`). A custom `sanitizer`/`markdownSanitizer` you pass is responsible for its own link hardening.
