...
|-------|------------|------|-------|
| `react-chorus` (`<Chorus>`) | 232.8 kB | 74.2 kB | Full widget path; includes Markdown parsing/sanitization and icons. |
| `react-chorus/headless` | 233.1 kB | 74.3 kB | Headless defaults, same behavior surface. |
| `react-chorus` (`useChorusStream`) | 72.0 kB | 21.4 kB | Root hook import; CI fails if it pulls UI, Markdown, or icon dependencies. |
| `react-chorus` (`Markdown`) | 76.1 kB | 25.7 kB | Standalone Markdown renderer; includes Markdown parsing/sanitization, not chat icons. |
| `react-chorus` (`ChatWindow`) | 133.8 kB | 44.2 kB | Transcript renderer with Markdown and message action icons, without the composer/widget shell. |
