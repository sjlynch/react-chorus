...
- `attachments.ts`, `metadata.ts`, `toolOutput.ts`, and `options.ts` — shared helpers for attachment source resolution/fallbacks, provider metadata aliases, tool output rendering, option stripping, and provider system precedence.
