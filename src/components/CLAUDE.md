...
When `message.speaker` is set, the bubble renders a small visible `MessageSpeakerBadge` (name + optional avatar) above the message content. The avatar `<img>` is gated by `showSpeakerAvatars` — the speaker name itself renders unconditionally. The SR-only label still announces the speaker once via `MessageSpeakerLabel`; the badge is `aria-hidden` to avoid duplicate reads. Designed for roleplay/multi-agent surfaces where several characters share `role: 'assistant'`.

The reasoning details block starts collapsed, but is auto-opened while a reasoning-only turn is still streaming so chain-of-thought is visible as it arrives. That auto-open is only a starting suggestion: `MessageReasoning` tracks the reader's own toggle in state, so a reader who collapses the chain-of-thought mid-stream keeps it collapsed as further chunks arrive (a controlled `open={true}` would re-force it open each chunk). The auto-open hint is also latched — `MessageReasoning` keeps an auto-opened panel open after the hint clears (which happens the instant answer text arrives), so the chain-of-thought a reader is following does not collapse out from under them; only an explicit reader collapse closes it.
