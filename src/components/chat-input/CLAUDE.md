...
- `DropOverlayPortal.tsx` — the "Drop to attach" drag overlay; portals onto the surrounding `.chorus` surface when one exists (re-resolved on every render so a ChatInput re-parented into/out of a surface keeps the overlay correct) and otherwise renders inside the composer root.
...
- `useAttachmentDragState.ts` — drag-depth state plus window dragend/blur cleanup. Keeps **separate** depth counters for the surface listeners (`surfaceDrag`) and the composer React handlers (`composerDrag`) so the two listener sets never decrement a count the other incremented; `draggingFiles` is the OR of both depths.
