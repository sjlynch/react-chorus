...
`isChorusDevMode()` is the shared dev-warning gate for modules that can import this util. `ChatWindow.tsx` intentionally keeps a local duplicate to avoid coupling hook-only chunks to ChatWindow code, while `ConversationList.tsx` uses the shared helper; keep new diagnostics behind one of these gates.
