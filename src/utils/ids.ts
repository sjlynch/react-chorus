...
function createProcessNonce(): string {
  ...
  if (typeof getRandomValues === 'function') {
    // Fill in place: `getRandomValues.call(...)` returns the loosely-typed
    // `ArrayBufferView` of the unbound method, so keep our own `Uint8Array`
    // reference for the iteration below.
