...
export function useAssistantSession<TMeta = Record<string, unknown>>({
  ...
  }, [appendAssistantNow, beginAssistantSession, completeActiveSession, createSessionHelpers, doStream, fallbackErrorMessageRef, historyForTransport, invalidateAssistantSession, isAssistantSessionActive, minAssistantDelayMsRef, messagesRef, onSendRef, rememberSubmittedTurn, removePendingAssistant, resetStreamState, safeOnError, setInternalSending, setTransportBusy, transportRef, updateSessionMessages]);
  ...
  const clear = React.useCallback(() => {
    ...
    updateSessionMessages(() => next, { flushPersistence: true });
    onClearRef.current?.(next);
  }, [isBusy, onClearRef, resetToInitialMessages, seedMessagesRef, stopActiveAssistant, updateSessionMessages]);
