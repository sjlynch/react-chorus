...
- `transport.ts` is the string URL shortcut. It deliberately duplicates the tiny default fetch request shape instead of importing the public transport chunk, preserving the transport subpath budget; keep it aligned with `createFetchSSETransport` when the default body changes.
