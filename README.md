...
```bash
# Install deps and build the library first
npm install
npm run build
...
|-------|------------|------|-------|
| `react-chorus` (`<Chorus>`) | 227.3 kB | 72.4 kB | Full widget path; includes Markdown parsing/sanitization and icons. |
| `react-chorus/headless` | 227.6 kB | 72.6 kB | Headless defaults, same behavior surface. |
| `react-chorus` (`useChorusStream`) | 68.9 kB | 20.6 kB | Root hook import; CI fails if it pulls UI, Markdown, or icon dependencies. |
| `react-chorus` (`Markdown`) | 76.1 kB | 25.7 kB | Standalone Markdown renderer; includes Markdown parsing/sanitization, not chat icons. |
| `react-chorus` (`ChatWindow`) | 132.4 kB | 43.6 kB | Transcript renderer with Markdown and message action icons, without the composer/widget shell. |
...
| `react-chorus/provider-requests` | 12.4 kB | 3.9 kB | Provider request mappers and tool serializers; no React/UI/Markdown runtime. |
