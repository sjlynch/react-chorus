...
- **Generative-UI blocks** — register components in `blocks` and the assistant emits `__render_block` calls that mount your component inline with streamed props plus an `emit` channel for follow-up turns or tool calls. Starter blocks (`Card`, `Form`, `Table`, `Image`, `CodeBlockComponent`, `Diff`, `CalendarPicker`), validator adapters (`zodAdapter`/`valibotAdapter`/`jsonSchemaAdapter`), and tool loaders ship as tree-shakeable subpaths — see the [`react-chorus/blocks` subpath docs](./docs/api.md#react-chorusblocks) and the [`examples/with-blocks`](./examples/with-blocks) runnable example.
- **Live cost meter** — `showCost` reads connector-emitted `metadata.usage`, looks up a model in the built-in `PRICING` snapshot (importable from [`react-chorus/pricing`](./docs/api.md#react-choruspricing) so you can override entries from your billing system), and renders a per-bubble chip plus a conversation total; `budgetAlert` trips a one-shot callback. The meter attaches usage automatically on the `transport` path; `onSend` flows supply `metadata.usage` themselves — via `helpers.finalizeAssistant({ metadata: { usage } })` or, on a bridged send, `helpers.streamCallbacks().onMetadata`. See the [cost-meter `onSend` recipe](./docs/api.md#cost-meter-showcost-pricing-budgetalert).
- **Roleplay / multi-character primitives** — `MessageSpeaker` on messages plus `<Chorus showSpeakerAvatars>` for visible name + avatar; `transformRequest` for pre-send context augmentation (lorebook injection, RAG retrieval, author's notes) without contaminating the transcript; `conversationMetadata` (and the underlying `useConversationMetadata` hook) for a small persisted slot per conversation. Designed so a downstream package can build character cards, lorebook editors, persona switchers, and group-chat orchestration on top of `react-chorus` rather than inside it — see the [`examples/with-roleplay`](./examples/with-roleplay) runnable example and the [roleplay primitives docs](./docs/api.md#messagespeaker-and-showspeakeravatars).
...
| [`examples/with-roleplay`](./examples/with-roleplay/README.md) | `MessageSpeaker` + `showSpeakerAvatars` + `transformRequest` + `conversationMetadata` end-to-end: two characters, a persona, a keyword-triggered lorebook injected per turn, and persisted active-character selection — template for a downstream roleplay layer |
...
- **[Connector capabilities](./docs/connectors.md)** — provider parity matrix for usage metadata, reasoning, citations/sources, tool calls, multimodal input/output, and code-execution limitations.
