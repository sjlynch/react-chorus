...
|-------|------------|------|-------|
| `react-chorus` (`<Chorus>`) | 182.9 kB | 60.4 kB | Full widget path; includes Markdown parsing/sanitization and icons. |
| `react-chorus/headless` | 183.3 kB | 60.6 kB | Headless defaults, same behavior surface. |
| `react-chorus` (`useChorusStream`) | 48.7 kB | 15.3 kB | Root hook import; CI fails if it pulls UI, Markdown, or icon dependencies. |
| `react-chorus` (`Markdown`) | 75.5 kB | 25.5 kB | Standalone Markdown renderer; includes Markdown parsing/sanitization, not chat icons. |
| `react-chorus` (`ChatWindow`) | 119.3 kB | 39.8 kB | Transcript renderer with Markdown and message action icons, without the composer/widget shell. |
| `react-chorus` (`ConversationList`) | 7.2 kB | 2.4 kB | Conversation sidebar component only; no Markdown/icon graph. |
| `react-chorus/transport` | 5.5 kB | 2.4 kB | Transport factories only; no React/UI/Markdown runtime. |
| `react-chorus/provider-requests` | 10.1 kB | 3.0 kB | Provider request mappers and tool serializers; no React/UI/Markdown runtime. |
