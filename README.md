...
|-------|------------|------|-------|
| `react-chorus` (`<Chorus>`) | 213.3 kB | 68.6 kB | Full widget path; includes Markdown parsing/sanitization and icons. |
| `react-chorus/headless` | 213.6 kB | 68.8 kB | Headless defaults, same behavior surface. |
| `react-chorus` (`useChorusStream`) | 65.3 kB | 19.4 kB | Root hook import; CI fails if it pulls UI, Markdown, or icon dependencies. |
| `react-chorus` (`Markdown`) | 76.0 kB | 25.7 kB | Standalone Markdown renderer; includes Markdown parsing/sanitization, not chat icons. |
| `react-chorus` (`ChatWindow`) | 127.9 kB | 42.5 kB | Transcript renderer with Markdown and message action icons, without the composer/widget shell. |
...
| `react-chorus/provider-requests` | 11.3 kB | 3.6 kB | Provider request mappers and tool serializers; no React/UI/Markdown runtime. |
