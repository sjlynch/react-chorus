...
async function verifyConsumerBundleBudgets() {
  ...
  const measurements = {};
  const entryBudgets = [
    { label: 'root entry initial JS', entry: 'root', maxSize: 196 * KiB, maxGzip: 65 * KiB },
    { label: 'headless entry initial JS', entry: 'headless', maxSize: 196 * KiB, maxGzip: 65 * KiB },
    { label: 'root useChorusStream import initial JS', entry: 'rootUseChorusStream', maxSize: 56 * KiB, maxGzip: 18 * KiB },
    { label: 'root Markdown import initial JS', entry: 'rootMarkdown', maxSize: 85 * KiB, maxGzip: 30 * KiB },
    { label: 'root ChatWindow import initial JS', entry: 'rootChatWindow', maxSize: 125 * KiB, maxGzip: 42 * KiB },
...
async function verifyPlaygroundBudgets() {
  ...
  const initialMaxSize = 417 * KiB;
  const initialMaxGzip = 132 * KiB;
  const measurements = {
