...
async function verifyConsumerBundleBudgets() {
  ...
  const measurements = {};
  const entryBudgets = [
    { label: 'root entry initial JS', entry: 'root', maxSize: 260 * KiB, maxGzip: 82 * KiB },
    { label: 'headless entry initial JS', entry: 'headless', maxSize: 260 * KiB, maxGzip: 82 * KiB },
    ...
    { label: 'root ChatWindow import initial JS', entry: 'rootChatWindow', maxSize: 144 * KiB, maxGzip: 47 * KiB },
...
async function verifyPlaygroundBudgets() {
  ...
  const initialMaxSize = 466 * KiB;
  const initialMaxGzip = 146 * KiB;
  const measurements = {
