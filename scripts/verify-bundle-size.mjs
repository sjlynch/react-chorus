...
async function verifyConsumerBundleBudgets() {
  ...
  const measurements = {};
  const entryBudgets = [
    { label: 'root entry initial JS', entry: 'root', maxSize: 280 * KiB, maxGzip: 88 * KiB },
    { label: 'headless entry initial JS', entry: 'headless', maxSize: 280 * KiB, maxGzip: 88 * KiB },
...
async function verifyPlaygroundBudgets() {
  ...
  const initialMaxSize = 530 * KiB;
  const initialMaxGzip = 164 * KiB;
  const measurements = {
